...
app.use('/*', async (c, next) => {
  ...
  // Si el origen está permitido, lo usamos, si no, usamos el primero de la lista (o ninguno)
...
app.post('/message', async (c) => {
  try {
    ...
    const { 
      ...
      system: frontendSystemPrompt // Recibimos el system prompt del frontend
    } = body;

    // A. Detectamos si el alumno está hablando de un tema específico para el progreso
    const lastUserMsg = messages.filter(m => m.role === 'user').pop()?.content || '';
    const detectedUnit = detectCurriculumUnit(subject, grade, lastUserMsg);

    // B. Usamos el prompt del frontend si existe, si no, usamos el del backend por defecto
